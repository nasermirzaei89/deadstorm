...
export class EnemySpawner {
    ...
    constructor(
        ...
        this.config = {
            ...defaults,
            ...config,
            timeline: (config.timeline ?? defaults.timeline).map((entry) => ({
                ...entry,
            })),
        };
        ...
        this.maxBurstsPerTick = Math.max(
            1,
            Number(this.config.maxBurstsPerTick ?? 8),
        );
        this.maxAccumulator = Math.max(
            1,
            Number(this.config.maxAccumulator ?? this.maxBurstsPerTick),
        );
    }
    ...
    update(timeMs, showEnemyHealthBars) {
        ...
        for (let i = 0; i < this.config.timeline.length; i += 1) {
            ...
            if (
                elapsedSeconds < entry.startAt ||
                elapsedSeconds >= entry.endAt
            ) {
                continue;
            }
            ...
            while (
                state.accumulator >= 1 &&
                burstsProcessed < this.maxBurstsPerTick
            ) {
                state.accumulator -= 1;
