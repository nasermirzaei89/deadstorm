    ...
    apply(bodyConfig) {
        ...
        switch (bodyConfig?.shape) {
            case 'circle': {
                ...
                const radius =
                    bodyConfig.radius ??
                    (bodyConfig.radiusFactor != null
                        ? defaultRadius * bodyConfig.radiusFactor
                        : defaultRadius);
                body.setCircle(
                    radius,
                    (baseWidth - radius * 2) / 2,
            ...
            default:
                const w =
                    bodyConfig?.width ??
                    (bodyConfig?.widthFactor != null
                        ? baseWidth * bodyConfig.widthFactor
                        : baseWidth);
                const h =
                    bodyConfig?.height ??
                    (bodyConfig?.heightFactor != null
                        ? baseHeight * bodyConfig.heightFactor
                        : baseHeight);
